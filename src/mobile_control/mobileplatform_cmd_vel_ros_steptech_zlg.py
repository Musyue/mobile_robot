...
import time
from mobile_control.mobileplatform_driver_steptech_zlg import *
import binascii
...
class MobilePlatFormKeyboardControl():
    def __init__(self,):
        ...
        self.CanAnalysisS=CanAnalysisDriverZlg()
        self.Abs_Encoder_fl_id1_oct=0
        self.Abs_Encoder_fr_id2_oct=0
        self.Abs_Encoder_rl_id3_oct=0
        self.Abs_Encoder_rr_id4_oct=0
        self.fl_abs_encode=self.CanAnalysisS.yamlDic['Homing_driver_fix_encoder_data']['fl']
        self.fr_abs_encode=self.CanAnalysisS.yamlDic['Homing_driver_fix_encoder_data']['fr']
        self.rl_abs_encode=self.CanAnalysisS.yamlDic['Homing_driver_fix_encoder_data']['rl']
        self.rr_abs_encode=self.CanAnalysisS.yamlDic['Homing_driver_fix_encoder_data']['rr']
...
"""
    ...
    def New_Read_Encoder_data_From_ABS_Encoder(self,RecNum):
        ...
        position_data_walk_vel_fl=self.MobileControl.Send_Control_Command(self.CanAnalysisS.yamlDic['sync_data_ID'],self.MobileControl.MobileDriver_Command.ZERO_COMMAND)
        # time.sleep(0.0015)

        ret,kk=self.CanAnalysisS.Can_New_Receive(0,RecNum)
        time.sleep(0.05)
        if ret:
    ...
    def Init_mobile_driver(self):
        self.CanAnalysisS.Init_Can_All()
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        recevenum=mpfh.CanAnalysisS.Can_GetReceiveNum(0)
        if recevenum!=None:
