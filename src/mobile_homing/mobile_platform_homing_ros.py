...
import rospy
import sys
from std_msgs.msg import String,Float64,Bool,Int32MultiArray
# sys.path.append("..")
from ctypes import *
import yaml
import os
...
class MobilePlatFormHoming():
    def __init__(self,):
        ...
        self.Pid=PID()
        self.Driver_encode_fl=0
        self.Driver_encode_fr=0
        self.Driver_encode_rl=0
        self.Driver_encode_rr=0
        ####ROS
    ...
    def New_Read_Encoder_data_From_ABS_Encoder(self,RecNum):
        ...
        time.sleep(0.05)
        #position feedback
        position_data_fl=self.MobileControl.Send_Control_Command(mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn1'],self.MobileControl.MobileDriver_Command.BASIC_LEFT_POSITION_FEEDBACK)
        time.sleep(0.05)
            ...
            if flag_1==1 and mpfh.Abs_Encoder_fr_id2_oct!=0 and mpfh.Abs_Encoder_fl_id1_oct!=0 and mpfh.Abs_Encoder_rr_id4_oct!=0 and mpfh.Abs_Encoder_rl_id3_oct!=0:
                ...
                if homing_ok_dic.has_key('fl') and homing_ok_dic.has_key('fr') and homing_ok_dic.has_key('rl') and homing_ok_dic.has_key('rr'):
                    if homing_ok_dic['fl']==1 and homing_ok_dic['fr']==1 and  homing_ok_dic['rl']==1 and  homing_ok_dic['rr']==1:  
                        flag_1=0
                        mpfh.MobileControl.logger.loggererror("---------------homing is ok------------------------")
