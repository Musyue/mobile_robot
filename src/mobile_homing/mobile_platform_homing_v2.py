...
class MobilePlatFormHoming():
    ...
    def Homing(self,feedback,abs_encode_oct,abs_encode_target,feedback_list,setpoint_list,time_list,Driver_id,Left_Right,count):
        self.Pid.update(feedback)
        output = self.Pid.output
        print str(Driver_id)+"output",output
        if self.Pid.SetPoint > 0:
            feedback =(abs_encode_oct*50)/1024# (output - (1/i))控制系统的函数
            print str(Driver_id)+"Feedback",feedback
        if count>1:
            self.Pid.SetPoint = (abs_encode_target*50)/1024
            print str(Driver_id)+"(abs_encode*50)/1024",(abs_encode_target*50)/1024
        self.MobileControl.Send_Velocity_Driver(output,Left_Right,Driver_id)
        time.sleep(0.01)
        feedback_list.append(feedback)
        setpoint_list.append(self.Pid.SetPoint)
        time_list.append(count)
        if abs(self.Pid.Error)<=1:
            self.MobileControl.Send_Velocity_Driver(0,Left_Right,Driver_id)
        return feedback_list,setpoint_list,time_list
    def Pid_control_Performance(self,feedback_list,setpoint_list,time_list,count,figure_num,END):
...
def main():
        ...
        for i in range(1, END):
            ...
            if recevenum!=None:
                if flag_1==1 and mpfh.Abs_Encoder_fr_id2_oct!=0 and mpfh.Abs_Encoder_fl_id1_oct!=0 and mpfh.Abs_Encoder_rr_id4_oct!=0 and mpfh.Abs_Encoder_rl_id3_oct!=0:
                    ...
                    # feedback_fl=
                    # =mpfh.Abs_Encoder_fr_id2_oct
                    feedback_list_fl,setpoint_list_fl,time_list_fl=mpfh.Homing(feedback_fl,mpfh.Abs_Encoder_fl_id1_oct,mpfh.fl_abs_encode,feedback_list_fl,setpoint_list_fl,time_list_fl,mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn1'],'left',i)
                    feedback_list_fr,setpoint_list_fr,time_list_fr=mpfh.Homing(feedback_fr,mpfh.Abs_Encoder_fr_id2_oct,mpfh.fr_abs_encode,feedback_list_fr,setpoint_list_fr,time_list_fr,mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn1'],'right',i)
                    feedback_list_rl,setpoint_list_rl,time_list_rl=mpfh.Homing(feedback_rl,mpfh.Abs_Encoder_rl_id3_oct,mpfh.rl_abs_encode,feedback_list_rl,setpoint_list_rl,time_list_rl,mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn2'],'left',i)
                    feedback_list_rr,setpoint_list_rr,time_list_rr=mpfh.Homing(feedback_rr,mpfh.Abs_Encoder_rr_id4_oct,mpfh.rr_abs_encode,feedback_list_rr,setpoint_list_rr,time_list_rr,mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn2'],'right',i)

                    #feedback,abs_encode_oct,abs_encode_target,feedback_list,setpoint_list,time_list,Driver_id,Left_Right,count
                    ...
                    #     feedback =(mpfh.Abs_Encoder_fl_id1_oct*50)/1024# (output - (1/i))控制系统的函数
                    #     print "Feedback",feedback_fl
                    # if i>1:
                    #     mpfh.Pid.SetPoint = (mpfh.fl_abs_encode*50)/1024
                    #     print "(mpfh.rl_abs_encode*50)/1024",(mpfh.fl_abs_encode*50)/1024
                    # mpfh.MobileControl.Send_Velocity_Driver(output,'left',mpfh.MobileControl.CanAnalysis.yamlDic['steering_channel']['chn1'])
                    # time.sleep(0.01)
                    # feedback_list_fl.append(feedback)
    ...
    mpfh.Pid_control_Performance(feedback_list_fl,setpoint_list_fl,time_list_fl,END) 
    mpfh.Pid_control_Performance(feedback_list_fr,setpoint_list_fr,time_list_fr,END) 
    mpfh.Pid_control_Performance(feedback_list_rl,setpoint_list_rl,time_list_rl,END) 
    mpfh.Pid_control_Performance(feedback_list_rr,setpoint_list_rr,time_list_rr,END)    
